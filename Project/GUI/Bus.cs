    ...
    public partial class Bus : Form
    {
        ...
        private void busUpdate()
        {
            if (isIntergerOrDoubleBL.isDouble(textBox7.Text))
            {
                ...
                cmd.CommandText = "Update Buses SET name = '" + textBox2.Text + "', address = '" + textBox10.Text + "', serviceProvider = '" + textBox4.Text + "', contractStart = '" + dateTimePicker1.Value + "', contractRenew = '" + dateTimePicker2.Value + "', discount = '" + Convert.ToDouble(textBox7.Text) + "', routesAvailaible = '" + textBox3.Text + "',busTypes = '" + textBox8.Text + "',busModels = '" + textBox9.Text + "' Where id = '" + checkId + "'";
                cmd.ExecuteNonQuery();
                sqlCon.Close();
                refreshList();
            }
        ...
        {
            //add
            if (textBox2.Text != "" && textBox10.Text != "" && textBox4.Text != "" && textBox7.Text != "" && textBox3.Text != "" && textBox9.Text != "" && textBox8.Text != "")
            {
                ...
                if (isIntergerOrDoubleBL.isDouble(textBox7.Text))
                {
                    ...
                    DTO.BusRoute = textBox3.Text;
                    DTO.BusType = textBox8.Text;
                    DTO.BusModel = textBox9.Text;

                    sqlCon.Open();
                    cmd.CommandText = "INSERT INTO Buses(name,address,serviceProvider,contractStart,contractRenew,discount,routesAvailaible,busTypes,busModels) VALUES ('" + DTO.CName + "','" + DTO.CAddress + "','" + DTO.CService + "','" + DTO.CContractStart + "','" + DTO.CRecontractRenew + "','" + DTO.CDiscount + "','" + DTO.BusRoute + "','" + DTO.BusType + "','" + DTO.BusModel + "')";
                    cmd.Connection = sqlCon;
                    cmd.ExecuteNonQuery();
                    sqlCon.Close();
                    refreshList();
        ...
        private void refreshList()
        {
            ...
            while (dr.Read())
            {
                ...
                myList.SubItems.Add(dr["routesAvailaible"].ToString());
                myList.SubItems.Add(dr["busTypes"].ToString());
                myList.SubItems.Add(dr["busModels"].ToString());
                busList.Items.Add(myList);
            }
            sqlCon.Close();
        ...
        private void busDelete()
        {
            sqlCon.Open();
            cmd.Connection = sqlCon;
            cmd.CommandText = "Delete From Buses Where id = '" + checkId + "'";
            cmd.ExecuteNonQuery();
            sqlCon.Close();
            refreshList();
        }
        ...
        private void busList_MouseClick(object sender, MouseEventArgs e)
        {
            checkId = busList.SelectedItems[0].SubItems[0].Text;
            sqlCon.Open();
            cmd.CommandText = "Select * From Buses Where id = " + checkId + "";
            cmd.Connection = sqlCon;
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                ...
                textBox3.Text = dr["routesAvailaible"].ToString();
                textBox8.Text = dr["busTypes"].ToString();
                textBox9.Text = dr["busModels"].ToString();
            }
            sqlCon.Close();
            button1.Enabled = false;
            button2.Enabled = true;
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (textBox1.Text != "")
            {
                ...
                {
                    cmd.CommandText = "SELECT * FROM Buses WHERE name LIKE '%" + textBox1.Text + "%'";
                }
                else
                {
                    cmd.CommandText = "SELECT * FROM Buses WHERE id LIKE '%" + textBox1.Text + "%'";
                }
                cmd.Connection = sqlCon;
                cmd.ExecuteNonQuery();
                SqlDataReader dr = cmd.ExecuteReader();
