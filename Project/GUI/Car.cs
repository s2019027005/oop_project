    ...
    public partial class Car : Form
    {
        ...
        private void carUpdate()
        {
            if (isIntergerOrDoubleBL.isDouble(textBox7.Text))
            {
                ...
                cmd.CommandText = "Update Cars SET name = '" + textBox2.Text + "', address = '" + textBox10.Text + "', serviceProvider = '" + textBox4.Text + "', contractStart = '" + dateTimePicker1.Value + "', contractRenew = '" + dateTimePicker2.Value + "', discount = '" + Convert.ToDouble(textBox7.Text) + "', citiesAvailaible = '" + textBox3.Text + "',driver = '" + textBox8.Text + "',carTypes = '" + textBox9.Text + "' Where id = '" + checkId + "'";
                cmd.ExecuteNonQuery();
                sqlCon.Close();
                refreshList();
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (textBox2.Text != "" && textBox10.Text != "" && textBox4.Text != "" && textBox7.Text != "" && textBox3.Text != "" && textBox9.Text != "" && textBox8.Text != "")
            {
                ...
                if (isIntergerOrDoubleBL.isDouble(textBox7.Text))
                {
                    ...
                    DTO.CarCity = textBox3.Text;
                    DTO.CarDriver = textBox8.Text;
                    DTO.CarType = textBox9.Text;

                    sqlCon.Open();
                    cmd.CommandText = "INSERT INTO Cars(name,address,serviceProvider,contractStart,contractRenew,discount,citiesAvailaible,driver,carTypes) VALUES ('" + DTO.CName + "','" + DTO.CAddress + "','" + DTO.CService + "','" + DTO.CContractStart + "','" + DTO.CRecontractRenew + "','" + DTO.CDiscount + "','" + DTO.CarCity + "','" + DTO.CarDriver + "','" + DTO.CarType + "')";
                    cmd.Connection = sqlCon;
                    cmd.ExecuteNonQuery();
                    sqlCon.Close();
                    refreshList();
        ...
        private void refreshList()
        {
            ...
            while (dr.Read())
            {
                ...
                myList.SubItems.Add(dr["citiesAvailaible"].ToString());
                myList.SubItems.Add(dr["driver"].ToString());
                myList.SubItems.Add(dr["carTypes"].ToString());
                carList.Items.Add(myList);
            }
            sqlCon.Close();
        ...
        private void carDelete()
        {
            sqlCon.Open();
            cmd.Connection = sqlCon;
            cmd.CommandText = "Delete From Cars Where id = '" + checkId + "'";
            cmd.ExecuteNonQuery();
            sqlCon.Close();
            refreshList();
        }
        ...
        private void carList_MouseClick_1(object sender, MouseEventArgs e)
        {
            checkId = carList.SelectedItems[0].SubItems[0].Text;
            sqlCon.Open();
            cmd.CommandText = "Select * From Cars Where id = " + checkId + "";
            cmd.Connection = sqlCon;
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                ...
                textBox3.Text = dr["citiesAvailaible"].ToString();
                textBox8.Text = dr["driver"].ToString();
                textBox9.Text = dr["carTypes"].ToString();
            }
            sqlCon.Close();
            button1.Enabled = false;
            button2.Enabled = true;
        ...
        private void textBox1_TextChanged_1(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                ...
                {
                    cmd.CommandText = "SELECT * FROM Cars WHERE name LIKE '%" + textBox1.Text + "%'";
                }
                else
                {
                    cmd.CommandText = "SELECT * FROM Cars WHERE id LIKE '%" + textBox1.Text + "%'";
                }
                cmd.Connection = sqlCon;
                cmd.ExecuteNonQuery();
                SqlDataReader dr = cmd.ExecuteReader();
